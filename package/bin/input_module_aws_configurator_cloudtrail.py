...
def collect_events(helper, ew):
    ...
    cloudtrail_username = 'audit_cloudtrail'
    ...
    cloudtrail_role_arn = helper.get_arg('aws_cloudtrail_role_arn')
    aws.configure_ta_cred(cloudtrail_username, cloudtrail_role_arn, ta_creds)


    service_name = 'cloudtrail_audit'
    service_definition = aws.find_in_dict(incremental_sqs_definitions, 'name', service_name)
    service_endpoint = 'splunk_ta_aws_aws_sqs_based_s3'

    service_config = {
        "name": service_name,
        "aws_account": "SplunkLoader-AWS",
        "aws_iam_role": cloudtrail_username,
        "interval" : "60",
        "sqs_batch_size" : "10",
        "s3_file_decoder" : "CloudTrail",
        "sqs_queue_url" : helper.get_arg('aws_cloudtrail_sqs_url'),
        "sqs_queue_region" : helper.get_arg('aws_cloudtrail_sqs_region'),
